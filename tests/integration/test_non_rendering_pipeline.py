...
COMPILER = ROOT / ".agents/skills/comfyui-workflow-compiler/scripts/compile_workflow.py"
...
class NonRenderingPipeline(unittest.TestCase):
    def test_ready_plan_compiles_without_gate_sidecar(self):
        plan_fixture = ROOT / "examples/general-idea/plan/production-plan-v01.yaml"
        with tempfile.TemporaryDirectory() as temp_dir:
            temp = Path(temp_dir)
            plan = yaml.safe_load(plan_fixture.read_text(encoding="utf-8"))
            plan_path = temp / "production-plan.yaml"
            ...
            result = subprocess.run(
                [sys.executable, str(COMPILER), "--plan", str(plan_path), "--catalog-root", str(ROOT / "workflow-catalog"), "--template-id", "h3-t2va-v1", "--bindings", str(bindings_path), "--capability-profile", str(profile_path), "--output", str(output), "--report", str(report)],
                capture_output=True,
                text=True,
            )
    ...
    def test_unusable_revision_and_over_cap_emit_no_workflow(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            temp = Path(temp_dir)
            ...
            self.assertIn("PLAN_REVISION_INVALID", result.stderr)
            self.assertFalse(output.exists())
            self.assertFalse(report.exists())
            plan["artifact"]["status"] = "ready"
            plan_path.write_text(yaml.safe_dump(plan), encoding="utf-8")
            output2, report2 = temp / "workflow-over-cap.json", temp / "report-over-cap.yaml"
            result2 = subprocess.run(
                [sys.executable, str(COMPILER), "--plan", str(plan_path), "--catalog-root", str(ROOT / "workflow-catalog"), "--template-id", "h3-t2va-v1", "--bindings", str(bindings_path), "--capability-profile", str(profile_path), "--output", str(output2), "--report", str(report2)],
                capture_output=True,
                text=True,
            )
