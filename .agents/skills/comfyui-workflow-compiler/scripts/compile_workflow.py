"""Live-profile-validated ComfyUI workflow compiler."""
...
def validate_plan(plan: dict):
    ...
    if not artifact.get("artifact_id") or not artifact.get("revision_id"):
        raise AssertionError("PLAN_REVISION_INVALID: artifact_id and revision_id are required")
    if artifact.get("status") in {"rejected", "superseded"}:
        raise AssertionError(f"PLAN_REVISION_INVALID: unusable status {artifact.get('status')}")
...
def validate_cinematic_plan(plan: dict, bindings: dict, capability_profile: dict | None = None) -> dict | None:
    ...
    for field in ("generation_relationship", "endpoint_policy"):
    ...
    if bindings["generation_relationship"] != handoff.get("relationship") or bindings["endpoint_policy"] != handoff.get("endpoint_policy"):
        raise AssertionError(f"GENERATION_RELATIONSHIP_INVALID: {segment_id} bindings do not match the plan handoff")
    ...
    if handoff.get("endpoint_policy") == "moving_endpoint":
        if not isinstance(handoff.get("motion_endpoint_evidence"), dict):
            raise AssertionError(f"MOVING_ENDPOINT_EVIDENCE_MISSING: {segment_id} has no endpoint evidence")
...
def validate_environment_projection(graph: dict, profile_path: Path) -> str:
    ...
    helper_path = Path(__file__).resolve().parents[2] / "minimax-h3-adapter" / "scripts" / "validate_environment_projection.py"
...
def main() -> int:
    ...
    if args.output.exists() or args.report.exists():
        raise AssertionError("ARTIFACT_OVERWRITE_FORBIDDEN")
    ...
    report = {
        ...
        "plan_id": plan["artifact"]["artifact_id"],
        "plan_revision": plan["artifact"]["revision_id"],
        "capability_profile_revision": profile.get("revision_id") or profile.get("probed_at") or "live-probe",
        "catalog_revision": catalog.get("revision_id") or catalog.get("version") or "declared-catalog",
    ...
    if cinematic_segment is not None:
        report.update({
            ...
            "camera_setup_id": next(item for item in plan["shots"] if item["shot_id"] == cinematic_segment["shot_id"])["camera"]["setup"]["setup_id"],
