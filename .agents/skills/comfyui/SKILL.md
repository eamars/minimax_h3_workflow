---
name: comfyui
description: Convert briefs, plots, storyboards, shot lists, and optional media references into ready-to-use, boundary-validated ComfyUI workflows; also inspect, validate, automate, and debug image, video, audio, and multimodal graphs. Use for MiniMax H3 multi-shot workflows that must generate and save a genuinely continuous final video from one Queue/Start action while preserving the verified local GPU, DynamicVRAM, and offload setup.
---
...
This route produces one UI-format workflow that runs every shot sequentially
and saves the final muxed video after one Queue Prompt action.

### Mandatory seamless-boundary contract
...
- Give every shot after the first an opening airlock of at least two seconds.
  Hold the predecessor's exact closing subject/prop state, camera/framing, and
  audio bed. Permit only natural micro-motion; start no action, dialogue, cut,
...
- Default to 8–12 second generated shots. A later shot needs at least six
  ...
  If user-locked timecodes make the boundary budget impossible, ask instead of
...
2. Compile a concise H3 action prompt for every generated shot, then author the
   structured continuity fields below. Make every later opening state/camera/
   audio value exactly equal to the preceding closing value. Do not rely on
   prose similarity.
3. Assign every shot an explicit duration. Honor supplied timing. Otherwise
   choose practical 8–12 second shots and preserve the requested total runtime.
     ...
     "shots": [
       {
         ...
         "continuity": {
           ...
           "opening_camera": "exact initial lens, position and framing",
           "opening_audio": "exact initial ambience and active sounds",
           "opening_hold_seconds": 0,
           "closing_scene": "exact established scene after any internal transition",
           "closing_state": "exact settled subject and prop arrangement",
           "closing_camera": "exact settled lens, position and framing",
           "closing_audio": "exact settled ambience and active sounds",
       ...
       },
       {
         ...
         "continuity": {
           ...
           "opening_camera": "exactly copy the previous closing_camera",
           "opening_audio": "exactly copy the previous closing_audio",
           "opening_hold_seconds": 2,
           "closing_scene": "next exact established scene",
           "closing_state": "next exact settled arrangement",
           "closing_camera": "next exact settled camera",
           "closing_audio": "next exact settled audio bed",
           "closing_hold_seconds": 2
         },
         "transition": {
...
the queue and logs, use the first-shot preview to catch a bad route early, and
  repair technical failures directly without asking the user to operate nodes.
