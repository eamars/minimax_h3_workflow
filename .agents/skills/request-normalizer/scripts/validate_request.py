...
def main() -> int:
    ...
    modes = {"FULL_PIPELINE", "PLAN_ONLY", "COMPILE_PLAN", "PATH_TEST", "REPAIR_ONLY", "ASSEMBLY_ONLY"}
    if execution.get("effective_mode") not in modes: fail("unsupported lifecycle mode")
    if execution.get("requested_lifecycle_mode") not in modes: fail("unsupported requested lifecycle mode")
    normalization = request["normalization"]
    if normalization.get("status") not in {"ready", "blocked", "failed"}: fail("invalid normalization status")
