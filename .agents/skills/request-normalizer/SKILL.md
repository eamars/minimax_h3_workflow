...
description: Convert a plain-language AI-video brief and supplied asset metadata into a conservative project request for the automatic production pipeline. Use first to normalize runtime, delivery constraints, exclusions, seed-asset declarations, and lifecycle preference before reference-canon-manager; never use it for plot, camera, image interpretation, prompt compilation, or rendering.
---
...
6. Copy supplied asset paths, types, and user roles into provisional `asset_inputs`; do not classify content.
7. Set `policy.max_generation_segment_seconds: 10` and preserve the requested lifecycle mode; default to `FULL_PIPELINE`.
...
10. Create a new revision and never overwrite an existing or superseded artifact.
...
- Preserve the requested lifecycle mode without inserting a production gate.
...
- Preserve stable IDs, provenance, source versions, and revisions.
- Let the orchestrator continue automatically after automated preflight.
- Keep ordering deterministic for identical semantic input.

## Non-responsibilities

Do not invent narrative events; decide actions or speech; interpret images; resolve canon conflicts; choose camera, shots, transitions, or segments; select MiniMax H3 modes; compile ComfyUI; probe or queue ComfyUI; validate outputs; reconcile conflicts silently; or modify upstream decisions.
...
- Require the ten-second policy and a recognized lifecycle mode.
- Require blocking decisions to make `handoff_ready` false.
- Reject traversal, absolute production paths, ambiguous `latest` aliases, duplicate IDs, and validated-artifact mutation.
...
- Preserve requested full-pipeline or partial lifecycle execution.
- Keep asset roles provisional and avoid visual interpretation.
- Emit stable IDs, provenance, revisions, and machine-readable failures.
